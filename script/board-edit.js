...
function renderAssignableContactsEdit(elementID) {
  ...
  const promises = allContacts[0].map((contact, index) => assignContactsTemplateEdit(contact.username, index, element, elementID));
...
function assignContactsTemplateEdit(username, index, element, elementID) {
  ...
  if (selectedClass === 'selectedContact') {
    checkboxImgElement.style.filter =
      'brightness(100) saturate(100%) invert(87%) sepia(14%) saturate(5010%) hue-rotate(541deg) brightness(250%) contrast(155%)';
  }
